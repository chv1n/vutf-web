            ...
            <table className="w-full hidden md:table">
                <thead>
                    <tr className="border-b border-gray-100">
                        <th className="text-left py-4 px-4 font-semibold text-gray-700 text-sm">รหัสวิทยานิพนธ์</th>
                        <th className="text-left py-4 px-4 font-semibold text-gray-700 text-sm">ชื่อวิทยานิพนธ์</th>
                        <th className="text-center py-4 px-4 font-semibold text-gray-700 text-sm">สมาชิก</th>
                        <th className="text-center py-4 px-4 font-semibold text-gray-700 text-sm">บทบาท</th>
                ...
                <tbody>
                    {activeGroups.map((group, index) => (
                        ...
                        >
                            ...
                            <td className="py-4 px-4">
                                <p className="text-gray-900 font-medium text-sm line-clamp-2 max-w-xs">{group.thesis_name_th}</p>
                                {group.status === 'rejected' && group.rejection_reason && (
                                    <div className="flex items-center gap-1 mt-1 text-red-500 bg-red-50 px-2 py-0.5 rounded-md w-fit border border-red-100">
                            ...
                            <td className="py-4 px-4">
                                <div className="flex items-center justify-center gap-1">
                                    {onView && (
                                        <button onClick={() => onView(group.group_id)} className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="ดูรายละเอียด">
                                            <FiEye className="w-4 h-4" />
                                        </button>
                                    )}
                                    {onEdit && group.role === 'owner' && group.status !== 'approved' && (
                                        <button onClick={() => onEdit(group.group_id)} className="p-2 text-gray-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="แก้ไข">
                                            <FiEdit className="w-4 h-4" />
                                        </button>
                                    )}
                                    {onDelete && group.role === 'owner' && (
                                        <button onClick={() => onDelete(group.group_id)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="ลบ">
                ...
                {activeGroups.map((group, index) => (
                    ...
                    >
                        <div className="flex justify-between items-start gap-2">
                            <span className="font-mono text-[12px] text-blue-600 bg-blue-50 px-2 py-0.5 rounded shrink-0">
                                {group.thesis_code}
                            </span>
                        ...
                        {group.status === 'rejected' && group.rejection_reason && (
                            <div className="flex items-start gap-1.5 p-2 bg-red-50 rounded-lg border border-red-100">
                                <FiAlertCircle className="w-3.5 h-3.5 text-red-500 shrink-0 mt-0.5" />
                                <p className="text-[11px] text-red-600 leading-tight">หมายเหตุ: กรุณารีบดำเนินการแก้ไขข้อมูล</p>
                            </div>
                        )}

                        <div className="flex items-center justify-end gap-2 pt-2 border-t border-gray-50">
                            {onView && (
                                <button onClick={() => onView(group.group_id)} className="flex-1 flex items-center justify-center gap-2 py-2 px-3 text-[12px] font-medium text-blue-600 bg-blue-50 rounded-lg">
                                    <FiEye className="w-3.5 h-3.5" /> รายละเอียด
                                </button>
                            )}
                            {onEdit && group.role === 'owner' && group.status !== 'approved' && (
                                <button onClick={() => onEdit(group.group_id)} className="p-2 text-amber-500 bg-amber-50 rounded-lg">
                                    <FiEdit className="w-4 h-4" />
                                </button>
                            )}
                            {onDelete && group.role === 'owner' && (
                                <button onClick={() => onDelete(group.group_id)} className="p-2 text-red-500 bg-red-50 rounded-lg">
                                    <FiTrash2 className="w-4 h-4" />
                                </button>
                            )}
                        </div>
